...
void logger(unsigned level, const char* msg, ...)
{
  ...
  // helper to print out just filename to console
...
Options:
  --output-gltf=<path>                Path to write bin/json to Sample ./models/
...
int main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (l != std::string::npos) {
      ...
      // VR: read out main groups so we can create file per site
      std::vector<std::string> siteNames;
      for (auto* srcRoot = store->getFirstRoot(); srcRoot != nullptr; srcRoot = srcRoot->next) {
          ...
          for (auto* srcModel = srcRoot->groups.first; srcModel != nullptr; srcModel = srcModel->next) {
              ...
              for (auto* srcModel = srcRoot->groups.first; srcModel != nullptr; srcModel = srcModel->next) {
                  ...
                  for (auto* srcGroup = srcModel->groups.first; srcGroup != nullptr; srcGroup = srcGroup->next) {
                      siteNames.push_back(srcGroup->model.project);
                  }
              }
          }
      }
    ...
    for (int i = 0; i < siteNames.size(); i++) {
        ...
        Flatten flattenMain(store);
        flattenMain.keepTag(siteNames[i].c_str());
        auto* storeNew = flattenMain.run();
         

              
        //Add colors
        Colorizer colorizer(logger, color_attribute.empty() ? nullptr : color_attribute.c_str());
        storeNew->apply(&colorizer);
        


        // logger ?
        connect(storeNew, logger);
        align(storeNew, logger);
        


        // groupbox
        AddGroupBBox addGroupBBox;
        storeNew->apply(&addGroupBBox);
        ...
        Flatten flatten(storeNew);
   


        if (chunkTinyVertexThreshold) {
            ChunkTiny chunkTiny(flatten, chunkTinyVertexThreshold);
            storeNew->apply(&chunkTiny);
            do_flatten = true;
        }
        ...
        AddStats addStats;
        storeNew->apply(&addStats);
        auto * stats = storeNew->stats;
        if (stats) {
            logger(0, "Stats:");
